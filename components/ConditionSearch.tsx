...
export function ConditionSearch({ value, onChange }: ConditionSearchProps) {
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      setIsLoading(true)
      try {
        const response = await fetch(`/api/suggestions?input=${encodeURIComponent(value)}&dictionary=Condition`)
        if (!response.ok) {
  ...
  return (
    <div className="relative">
      ...
      <Input
        id="condition"
        type="text"
        placeholder="Enter a condition or disease (e.g., Cancer)"
        value={value}
